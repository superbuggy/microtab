export const scalarIntervallicDistances24EDO = {
  semaphore5: [5, 5, 4, 5, 5],
  semaphore9: [4, 1, 4, 1, 4, 4, 1, 4, 1],
  maqamic7: [3, 4, 3, 3, 4, 3, 4],
  maqamic10: [3, 3, 1, 3, 3, 3, 1, 3, 3, 1],
  sensi5: [6, 3, 6, 6, 3],
  triforce6: [3, 5, 3, 5, 3, 5],
  triforce9: [3, 3, 2, 3, 3, 2, 3, 3, 2],
  triforce15: [1, 2, 1, 2, 2, 1, 2, 1, 2, 2, 1, 2, 1, 2, 2],
  "Enharmonic Mixolydian ": [1, 1, 8, 1, 1, 8, 4],
  "Enharmonic Lydian ": [1, 8, 1, 1, 8, 4, 1],
  "Enharmonic Phrygian ": [8, 1, 1, 8, 4, 1, 1],
  "Enharmonic Dorian ": [1, 1, 8, 4, 1, 1, 8],
  "Enharmonic Hypolydian": [1, 8, 4, 1, 1, 8, 1],
  "Enharmonic Hypophrygian": [8, 4, 1, 1, 8, 1, 1],
  "Enharmonic Hypodorian ": [4, 1, 1, 8, 1, 1, 8],

  "Soft Diatonic Mixolydian ": [2, 3, 5, 2, 3, 5, 4],
  "Soft Diatonic Lydian ": [3, 5, 2, 3, 5, 4, 2],
  "Soft Diatonic Phrygian ": [5, 2, 3, 5, 4, 2, 3],
  "Soft Diatonic Dorian ": [2, 3, 5, 4, 2, 3, 5],
  "Soft Diatonic Hypolydian ": [3, 5, 4, 2, 3, 5, 2],
  "Soft Diatonic Hypophrygian 4 2 3 5 2 3 5 Soft Diatonic Hypodorian ": [
    5, 4, 2, 3, 5, 2, 3,
  ],
  "Maqam Ouchairan-Hussaini, Bayatan, Neutral Diatonic Mixolydian ": [
    3, 3, 4, 3, 3, 4, 4,
  ],
  "Dastgah-e Sehgah, Neutral Diatonic Lydian ": [3, 4, 3, 3, 4, 4, 3],
  "Arabic Diatonic, Maqam Rast, Quasi-equal Heptatonic, Neutral Diatonic Phrygian ":
    [4, 3, 3, 4, 4, 3, 3],
  "Maqam Hussaini, Ushaq, Neutral Diatonic Dorian ": [3, 3, 4, 4, 3, 3, 4],
  "Maqam Sikah (Segah), Neutral Diatonic Hypolydian ": [3, 4, 4, 3, 3, 4, 3],
  "Neutral Diatonic Hypophrygian ": [4, 4, 3, 3, 4, 3, 3],
  "Miha'il Musaqa's mode: Egypt, Neutral Diatonic Hypodorian, Dastgah-e Sehgah, Maqam Nairuz ":
    [4, 3, 3, 4, 3, 3, 4],
  "Diatonic + Enharmonic Diesis Mixolydian ": [1, 5, 4, 1, 5, 4, 4],
  "Diatonic + Enharmonic Diesis Lydian ": [5, 4, 1, 5, 4, 4, 1],
  "Diatonic + Enharmonic Diesis Phrygian ": [4, 1, 5, 4, 4, 1, 5],
  "Diatonic + Enharmonic Diesis Dorian ": [1, 5, 4, 4, 1, 5, 4],
  "Diatonic + Enharmonic Diesis Hypolydian ": [5, 4, 4, 1, 5, 4, 1],
  "Diatonic + Enharmonic Diesis Hypophrygian ": [4, 4, 1, 5, 4, 1, 5],
  "Diatonic + Enharmonic Diesis Hypodorian ": [4, 1, 5, 4, 1, 5, 4],
  "Chromatic/Enharmonic Mixotydian ": [1, 3, 6, 1, 3, 6, 4],
  "Chromatic/Enharmonic ": [3, 6, 1, 3, 6, 4, 1],
  "Chromatic/Enharmonic Lydian Phrygian ": [6, 1, 3, 6, 4, 1, 3],
  "Chromatic/Enharmonic Dorian ": [1, 3, 6, 4, 1, 3, 6],
  "Chromatic/Enharmonic Hypolydian ": [3, 6, 4, 1, 3, 6, 1],
  "Chromatic/Enharmonic Hypophrygian ": [6, 4, 1, 3, 6, 1, 3],
  "Chromatic/Enharmonic Hypodorian ": [4, 1, 3, 6, 1, 3, 6],
  "Neutral Mixotydian ": [3, 4, 3, 3, 4, 3, 4],
  "Neutral Lydian ": [4, 3, 3, 4, 3, 4, 3],
  "Neutral Phrygian ": [3, 3, 4, 3, 4, 3, 4],
  "Neutral Dorian, Dudon's Mohajira, Misaelides 2nd Byzantine mode, Maqam Sikah ":
    [3, 4, 3, 4, 3, 4, 3],
  "Baladi Neutral Hypolydian ": [4, 3, 4, 3, 4, 3, 3],
  "Neutral Hypophrygian ": [3, 4, 3, 4, 3, 3, 4],
  "Neutral Hypodorian ": [4, 3, 4, 3, 3, 4, 3],
  "Anthanasopoulos' byzantine liturgical Chromatic dastgah-e Chahargah ": [
    3, 5, 2, 4, 3, 5, 2,
  ],
  "Dastgah·E Nava, Maqam ushaq masri ": [4, 2, 4, 4, 3, 3, 4],
  "Second plagal byzantine liturgical mode": [2, 7, 1, 4, 2, 7, 1],
  "Quasi-equal Pentatonic (from A) / Haba's Pentatonic I MOS Bug - 4L 1 s": [
    5, 5, 4, 5, 5,
  ],
  "Maqam 'Ushshaq turki, urfa, isfahan, Dastgah-e Shur": [3, 3, 4, 4, 2, 4, 4],
  "Maqam bayati": [3, 3, 4, 4, 2, 1, 3, 4],
  "Maqam nahfat": [3, 3, 4, 4, 4, 2, 4],
  "Maqam saba": [3, 3, 2, 6, 2, 4, 4],
  "Maqam saba 2": [3, 3, 2, 6, 2, 4, 2, 2],
  "Maqam sabr jadid": [3, 3, 2, 6, 2, 6, 2],
  "Maqam mansuri": [3, 3, 2, 2, 4, 3, 3, 4],
  "Maqam suznak (soznak)": [4, 3, 3, 4, 2, 6, 2],
  "Maqam rast, Dilkashidah, Dilnishin": [4, 3, 3, 4, 4, 2, 1, 3],
  "Maqam mahur": [4, 3, 3, 4, 4, 4, 2],
  "Maqam yakah": [4, 3, 1, 2, 1, 3, 4, 2, 1, 3],
  "Maqam suzidil'ara": [4, 3, 1, 2, 4, 4, 2, 4],
  "Maqam Qarjighar, Bayati shuri": [3, 3, 4, 2, 6, 2, 4],
  "Maqam Huzam": [3, 4, 2, 2, 2, 2, 2, 4, 3],
  "Maqam hizam (Huzam, el Houzam), rahat al arouah": [3, 4, 2, 6, 2, 4, 3],
  "Maqam Iraq": [3, 4, 3, 3, 4, 4, 2, 1],
  "Maqam Hijaz": [2, 6, 2, 4, 2, 1, 3, 4],
  "Maqam musta'ar": [3, 4, 4, 2, 1, 3, 4, 3],
  "Maqam farahnak": [3, 4, 4, 4, 2, 4, 2, 1],
  "Maqam bastanikar, tarz nuin": [3, 4, 3, 3, 2, 6, 2, 1],
  "Maqam farah faza, maqam nakriz": [4, 2, 6, 2, 4, 2, 1, 3],
  "Maqam hayyan": [4, 2, 1, 3, 2, 2, 2, 2, 3, 1, 2],
  "Maqam basandida": [4, 2, 1, 3, 2, 2, 4, 2, 1, 3],
  "Maqam Shawq afza": [4, 4, 2, 1, 3, 2, 2, 4, 2],
  "Maqam Shawq Tarab": [2, 1, 3, 2, 2, 4, 2, 4, 2, 2],
  "Maqam jabburi": [3, 1, 2, 4, 4, 2, 4, 4],
  "Maqam Nawa": [2, 4, 4, 4, 3, 3, 4],
  "Maqam higaz-kar": [2, 5, 3, 4, 2, 5, 3],
  "Maqam su-ar, naghmeh abuata, naghmeh afshari": [3, 4, 4, 2, 4, 4, 3],
  "Maqam jahargah (hiharkah), Naghmeh bayat-e tork Naghmeh dashti": [
    4, 4, 2, 4, 4, 3, 3,
  ],
  "Dastgah-e homayun": [3, 5, 2, 4, 2, 4, 4],
  "Naghmeh esfahan": [4, 2, 4, 4, 3, 5, 2],
  "Maqam 'awg' ara (aug-ara)": [3, 6, 1, 5, 2, 6, 1],
  "Maqam buselik": [4, 1, 5, 4, 2, 6, 2],
  "Maqam neuter": [4, 2, 6, 2, 2, 5, 3],
  "Anchihoye: Ethiopia": [2, 8, 3, 6, 5],
  "Dance scale of the yi people: China": [4, 3, 3, 4, 4, 2, 4],
  "Danial-mode of the spanish-arab Jews": [4, 4, 2, 3, 1, 4, 6],
  Spondeion: [3, 7, 4, 3, 7],
  Spondeiakos: [1, 1, 8, 4, 2, 8],
  "De vries 13-tone": [1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2],
  "De vries 9-tone": [4, 1, 4, 1, 4, 1, 4, 1, 4],
  "De vries 5-tone": [9, 2, 2, 9, 2],
  "Progressive Enneatonic": [1, 2, 3, 4, 4, 1, 2, 3, 4],
  "Young decatonic": [2, 3, 2, 3, 2, 2, 3, 2, 3, 2],
  "Young half octave diatonic": [2, 2, 1, 2, 2, 2, 1, 2, 2, 1, 2, 2, 2, 1],
  "Breed decatonic": [3, 1, 3, 3, 3, 1, 3, 3, 1, 3],
  "Oljare decatonic": [2, 3, 2, 2, 3, 2, 3, 2, 2, 3],
  "Ivan Wyschnegradsky's diatonicized chromatic scale - MOS 11 L 2s (Agmon diatonic D85)":
    [2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1],
  "Maqam Rahat al-Arwah": [3, 4, 2, 6, 2, 4, 2, 1],
  Ionian: [4, 4, 2, 4, 4, 4, 2],
  Dorian: [4, 2, 4, 4, 4, 2, 4],
  Phrygian: [2, 4, 4, 4, 2, 4, 4],
  Lydian: [4, 4, 4, 2, 4, 4, 2],
  Mixolydian: [4, 4, 2, 4, 4, 2, 4],
  Aeolian: [4, 2, 4, 4, 2, 4, 4],
  Locrian: [2, 4, 4, 2, 4, 4, 4],
};

const intervallicDistances = (pitchNumbers) =>
  pitchNumbers.slice(1).reduce((intervals, pitchNumber, index) => {
    intervals.push(pitchNumber - pitchNumbers[index]);
    return intervals;
  }, []);

// const compareCentsForEDO = (centsValues, edo) =>
//   centsValues.map((i) => Math.round(i / (1200 / edo)) * (1200 / edo) - i);

const centsToEDO = (centsValues, edo) =>
  centsValues.map((i) => Math.round(i / (1200 / edo)));

const approxBohlenPierceCents = [
  0, 133.2, 301.8, 435.1, 582.5, 736.9, 884.4, 1017.0, 1165.0, 1319.4, 1466.9,
  1600.1, 1768.7, 1900,
];
const modifiedBohlenPierceCents = [
  0, 133.2, 301.8, 435.1, 590.2, 745.8, 884.4, 1035.0 /* neutral 7th */, 1165.0,
  1328.3, 1453.8, 1600.1, 1768.7, 1900,
];

const overTonePrimes = [0, 386, 702, 969, 551, 840, 105, 297, 1200].sort(
  (a, b) => a - b
); // octave reduced

scalarIntervallicDistances24EDO["Bohlen-Pierce 24 EDO Approximation"] =
  intervallicDistances(centsToEDO(approxBohlenPierceCents, 24));
scalarIntervallicDistances24EDO["Bohlen-Pierce 24 EDO (Neutral 7th)"] =
  intervallicDistances(centsToEDO(modifiedBohlenPierceCents, 24));
scalarIntervallicDistances24EDO["Prime Overtone 24 EDO"] = intervallicDistances(
  centsToEDO(overTonePrimes, 24)
);
