...
export function usePitchDistance() {
  ...
  const nearestNote = computed(() => {
    if (!inputPitch.value) return null;
    return findNearestNeighbor(inputPitch.value, frequencyDict.value);
  });
  ...
  const centsToNearest = computed(() => {
    if (!inputPitch.value || !nearestNote.value) return null;
    const frequencyRatio = inputPitch.value / nearestNote.value.hz;
